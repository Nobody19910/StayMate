        ...
        <div className="max-w-6xl mx-auto space-y-8 flex-1 flex flex-col w-full pb-20 md:pb-0">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {liveHomes.length === 0 ? <p className="text-sm text-gray-400">None live.</p> : liveHomes.map(h => (
                      <div key={h.id} className="border border-gray-100 p-3 rounded-2xl hover:bg-gray-50">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0 ml-2">
                            ...
                            <button
                              ...
                              className={`text-[9px] font-bold px-2 py-1 rounded-lg transition-all active:scale-95 ${h.is_verified ? "text-white" : "text-gray-500 border border-gray-200"}`}
          ...
          {tab === "featured" && (
            <div className="bg-white border border-gray-100 p-6 rounded-3xl shadow-sm">
              ...
              {/* Homes */}
              <h3 className="text-xs font-bold text-emerald-600 uppercase tracking-widest mb-3 px-1">Homes</h3>
              <div className="space-y-2 mb-8">
                {liveHomes.length === 0 ? <p className="text-sm text-gray-400">No live homes.</p> : liveHomes.map(h => {
                  ...
                  return (
                    <div key={h.id} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-gray-900 line-clamp-1">{h.title}</p>
                        <p className="text-xs text-gray-500">{h.city} • {h.price_label}</p>
                      </div>
                      <button
                        onClick={async () => {
              ...
              {/* Hostels */}
              <h3 className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-3 px-1">Hostels</h3>
              <div className="space-y-2">
                {liveHostels.length === 0 ? <p className="text-sm text-gray-400">No live hostels.</p> : liveHostels.map(h => {
                  const isFeatured = !!h.is_sponsored;
                  const totalFeatured = homes.filter(x => x.is_sponsored).length + hostels.filter(x => x.is_sponsored).length;
                  return (
                    <div key={h.id} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-gray-900 line-clamp-1">{h.name}</p>
                        <p className="text-xs text-gray-500">{h.city} • {h.price_range_label}</p>
                      </div>
                      <button
                        onClick={async () => {
                          if (!isFeatured && totalFeatured >= 10) { alert("Maximum 10 featured properties allowed."); return; }
                          ...
                          >
                            ...
                            <div className="w-full space-y-2">
                              ...
                              <div className="flex gap-2 pt-2 flex-wrap">
                                ...
                                {agent.is_agent && (
                                  <span className="text-[10px] font-bold bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full">Subscribed Agent</span>
                                )}
                                {(() => {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap md:whitespace-normal shrink-0 border ${
        disabled
        ? "bg-transparent text-gray-600 border-transparent opacity-40 cursor-not-allowed"
        : active
    ...
    >
      ...
      {disabled && (
        <span className="px-2 py-0.5 text-[9px] rounded-full shrink-0 bg-gray-700 text-gray-400">Soon</span>
      )}
